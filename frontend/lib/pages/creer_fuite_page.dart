...
class _CreerFuitePageState extends State<CreerFuitePage> {
  ...
  Future<AnalyseIAReponse?> _analyserPhotos() async {
    if (_photoPaths.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Ajoute d\'abord des photos à analyser'),
    ...
    try {
      ...
      if (_fuiteIdApresSave == null) {
        await _sauvegarderAvantIA();
        if (!mounted) return null;
      }

      // ── Étape 2 : analyser ──
      final reponse = await AnalyseIAService.analyserParFuite(
        fuiteId: _fuiteIdApresSave!,
      );
        ...
        SnackBar(
          content: Row(
            children: [
              ...
              const SizedBox(width: 10),
              Text(
                '${reponse.resultats.length} média(s) analysé(s) — Diamètre : ${reponse.resume.diametreMoyenMm.toStringAsFixed(1)} mm',
  ...
  /// Stocke l'ID dans [_fuiteIdApresSave] pour l'analyse IA.
  Future<void> _sauvegarderAvantIA() async {
    ...
    final nouvelleFuite = await fuite_api.createFuite(
      campagneId: _selectedCampagneId ?? _campagnes.first.id,
    ...
    _fuiteIdApresSave = nouvelleFuite.id;
        ...
        SnackBar(
          content: const Row(
            children: [
              ...
              SizedBox(width: 10),
              Text('Fuite sauvegardée — analyse IA en cours…'),
            ],
          ),
          backgroundColor: const Color(0xFF00875A),
          behavior: SnackBarBehavior.floating,
    ...
    try {
      ...
      if (_fuiteIdApresSave != null) {
        await fuite_api.updateFuite(
          id: _fuiteIdApresSave!,
          campagneId: _selectedCampagneId!,
          numeroTag: _tagCtrl.text.trim().isEmpty ? null : _tagCtrl.text.trim(),
          dateDetection:
              '${_dateCtrl.text.trim().replaceFirst(' ', 'T')}.000000',
          statut: _statut,
          pressionBar: pression,
  ...
  /// Carte de résultat persistant après analyse IA
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Icon(Icons.image_rounded, size: 14, color: Colors.grey.shade500),
              const SizedBox(width: 6),
              Text(
                '${reponse.resultats.length} analysé(s)',
                style: TextStyle(fontSize: 12, color: Colors.grey.shade600),
              ),
              const SizedBox(width: 12),
              Icon(
              ...
              const SizedBox(width: 12),
              Icon(Icons.cancel_rounded, size: 14, color: Colors.grey.shade400),
              const SizedBox(width: 4),
              Text(
